...
def get_spy_price() -> float:
    ...
    snapshot = data_client.get_stock_latest_quote(request)[UNDERLYING]
...
def get_option_contracts(expiry_date: str) -> list:
    request = GetOptionContractsRequest(underlying_symbols=[UNDERLYING], expiration_date=expiry_date, limit=200, )
...
def select_legs(contracts: list, price: float):
    ...
    if not short_put or not short_call: return None, None, None, None
    ...
    if not long_put_candidates or not long_call_candidates: return None, None, None, None
    ...
    log.info(f"Legs — Long Put: {long_put.strike_price} | Short Put: {short_put.strike_price} | Short Call: {short_call.strike_price} | Long Call: {long_call.strike_price}")
    return short_put, long_put, short_call, long_call

def place_order(symbol: str, side: OrderSide):
    order = client.submit_order(MarketOrderRequest(symbol=symbol, qty=QTY, side=side, time_in_force=TimeInForce.DAY, ))
